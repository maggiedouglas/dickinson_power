...
```{r grid, fig.align = 'center', out.width = "90%", fig.cap = "Main meter buildings shown in color (diagram courtesy of Mike Kiner)", fig.topcaption=TRUE}
knitr::include_graphics("assets/electrical-grid.png", error = FALSE)
```

## Background

The "main meter" records electricity use in around 40 buildings in the core of campus. These buildings have a range of uses including academic, administrative, and residential functions. Importantly, the main meter also includes the Central Energy Plant in Kaufman Hall. Together, the buildings on the main meter account for 59% of square footage on campus and 68% of electricity use at an estimated cost of $1.2 million and 3504 MT CO2-e in fiscal year 2025 (FY25; see [Electricity summary: Campus summary](https://maggiedouglas.github.io/dickinson_power/campus_summary.html)). In FY25 the buildings on the main meter used 10.4 kWh per square foot, very similar to values typical of U.S. College/University buildings (median 10.3 kWh/sqft/yr; EIA 2022). 
...
```{r check_seg, fig.height = 4, fig.width = 8, include = FALSE}
...
```{r results segmented, message = FALSE, include = FALSE}
