...
# load temp data
temp <- read.csv("./keys/temp_by_date.csv") %>%
  mutate(date = mdy(date))

# attaching all dataframes together 
## (reads in two kWh columns b/c slightly diff names)
main_bldgs <- bind_rows(data, .id="Sheet") %>%
  rename(building = Sheet,
         date = Date, 
         kWh1 = 3,
         kWh2 = 4) %>%
  mutate(kwh = ifelse(is.na(kWh1), kWh2, kWh1), # reconcile kWh to condense to one column
         date = mdy(date)) %>%
  filter(date < ymd("2025-07-01"),  # filter to fiscal year 25
         date > ymd("2024-06-30")) %>%
  select(-kWh1, -kWh2) %>% # remove extra kWh columns
  mutate(month = month(date, label = TRUE), 
         day = wday(date, label = TRUE)) %>% # generate month and day
  select(building, month, day, date, kwh) %>%
  left_join(temp, by = "date") # add back in temperature + DD
```
...
annual <- main_bldgs %>%
  group_by(building) %>%
  summarize(kwh = round(sum(kwh, na.rm = T), digits = 0),
            days = n()) %>%
...
# What percentage of the total is captured here?
(indiv_tot / main_tot)*100
```

## Summary graphs

```{r graph, warning = F, fig.width = 12, fig.height = 10}
...
```{r table daily}
datatable(select(main_bldgs, building, date, kwh, ave_temp),
          filter = 'top', rownames = FALSE, colnames = c("Building","Date","kWh", "Temp (F)"))
```
