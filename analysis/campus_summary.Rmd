...
```{r submeter_stacked, warning = F, fig.height = 4.5, fig.height = 4.5}
ggplot(filter(daily_graph, meter == "Submeter"),
       aes(x = month, y = kwh/10^6, fill = reorder(type_brief, kwh, FUN = sum))) +
  geom_col(position = "stack") +
  scale_fill_brewer(type = "qual", palette = "Paired") +
...
```{r stacked bar agg, warning = F, fig.height = 3.5, fig.width = 7}
...
```{r stacked bar indiv, warning = F, fig.height = 6, fig.width = 8}
...
```{r stacked bar sub, warning = F, fig.height = 4, fig.width = 6}
ggplot(filter(daily_graph, meter == "Submeter"),
       aes(x = month, y = kwh/10^3, fill = reorder(type, kwh, FUN = 'sum'))) +
  geom_col(position = "stack") +
  facet_wrap(. ~ type, scales = "free") +
...
## Electricity intensity by meter + building type
