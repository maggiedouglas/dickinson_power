...
buildings_clean <- buildings %>%
  ...
  mutate(meter = ifelse(weis_meter == 1, "Weis Meter", 
                               ifelse(main_meter == 1, "Main Meter", "Individual"))) %>%
...
joined <- kwh_annual %>%
  ...
  filter(!meter %in% c("Main Meter", "Weis Meter")) %>% # filter out those on main or Weis meter (not disagg)
  ...
  select(type, meter, NAME, days_perc, kwh, kwh_sqft, kwh_person, dollars, ghg_kgCO2)
...
ggplot(filter(buildings_sum, ! type_new %in% c("Res Hall - U","Non-building","Production")),
...
```{r export}
write.csv(joined_daily_full, "./output/kwh_daily.csv", row.names = F)
write.csv(joined_full, "./output/kwh_annual.csv", row.names = F)
```
